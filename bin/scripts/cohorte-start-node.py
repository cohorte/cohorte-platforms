...
def get_external_config(parsed_conf_file, conf_name):
    ...
    """
    if parse_config_file and conf_name:
        if "node" in parsed_conf_file:
            ...
            if conf_name == "is-top-composer":
                ...
                if "web-admin-port" in parsed_conf_file["node"].keys():
                    return parsed_conf_file["node"]["web-admin-port"]
            elif conf_name == "shell-admin-port":
                if "shell-admin-port" in parsed_conf_file["node"].keys():
...
def main(args=None):
    ...
    group.add_argument("--config-file", action="store", default="run.js",
    ...
    if not "-t" in boot_args:
        is_top = get_external_config(external_config, "is-top-composer")
        if is_top:
            boot_args.append("-t")
    if "-t" in boot_args:
