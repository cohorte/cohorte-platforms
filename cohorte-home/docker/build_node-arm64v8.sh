...
DIRECTORY="/root/docker/node-arm64v8"
...
echo "scp -r -o StrictHostKeyChecking=no -i $ID_RSA $DIR/node-arm64v8 root@$DOCKER_ARM_HOST:/root/docker/node-arm64v8"
ssh -o StrictHostKeyChecking=no -i $ID_RSA  root@$DOCKER_ARM_HOST 'rm -r /root/docker/node-arm64v8' 
scp -r -o StrictHostKeyChecking=no -i $ID_RSA $DIR/node-arm64v8 root@$DOCKER_ARM_HOST:/root/docker/node-arm64v8
